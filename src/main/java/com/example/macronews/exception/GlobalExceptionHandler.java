package com.example.macronews.exception;

import com.example.macronews.dto.response.ErrorResponse;
...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleHttpRequestMethodNotSupported(
        ...
        // 로그에는 사용자 입력이 섞일 수 있는 URI 등을 안전하게 정리해서 출력
        log.warn(
        ...
        // 응답에는 예외 메시지(ex.getMessage())를 그대로 노출하지 않음(XSS/정보노출 방지)
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(
        ...
        // defaultMessage는 보통 서버 정의 문구지만, 안전을 위해 응답 문자열은 escape 처리
        String message = ex.getBindingResult().getFieldErrors().stream()
                .map(error -> safeForResponseText(error.getField()) + "=" + safeForResponseText(
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(
        ...
        // payload 파싱 실패 시에도 예외 메시지를 그대로 노출하지 않음
        ErrorResponse error = new ErrorResponse(
                HttpStatus.BAD_REQUEST.value(),
    ...
    /**
     * path 표시 시 사용자 입력이 섞일 수 있으므로 escape 처리
     */
    ...
    /**
     * 스택트레이스 출력은 최소화하고 throwable.getMessage()는 포함하지 않음
